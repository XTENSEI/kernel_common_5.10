 ...
 *
 * Tuned to 16 ms (constant) for Android UI smoothness; the upstream CFS
 * default is 6 ms * (1 + ilog(ncpus)).
 */
 ...
 *
 * Tuned to NONE (constant) for Android UI smoothness; the upstream CFS
 * default is LOG.
 */
 ...
 *
 * Tuned to 2 ms (constant) for Android UI smoothness; the upstream CFS
 * default is 0.75 ms * (1 + ilog(ncpus)).
 */
 ...
 *
 * On Android, setting this to 1 improves app-launch latency:
 * when zygote forks a new app process, the child gets the CPU
 * immediately instead of waiting for the parent to yield.
 */
